"""
Інтерактивний графік прийому ліків v8
...
- Вкладка «Фінанси» — облік витрат на ліки
...
STATE_FILE = os.path.join(SCRIPT_DIR, "med_state_v8.json")
...
class MedicationApp:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Графік прийому ліків v8 — Бучин А.П.")
    ...
    def _build_ui(self):
        ...
        self.notebook.add(self.tab_finance, text="  Фінанси  ")
    ...
    def _build_finance_tab(self):
        # All unique medications for rows
        self._fin_meds = []
        seen = set()
        for sec in MEDICATIONS:
            for it in sec["items"]:
        ...
        # Load finance data: {"dates": ["01.04", ...], "cells": {"med_id": {"01.04": 123.5, ...}}}
        self._fin_data = self.state.get("_finance", {"dates": [], "cells": {}})

        # Header
        hdr = tk.Frame(self.tab_finance, bg=C["header_bg"])
        hdr.pack(fill="x")
        tk.Label(hdr, text="ОБЛІК ВИТРАТ НА ЛІКИ", bg=C["header_bg"], fg=C["header_fg"],
        ...
        add_btn = tk.Label(tb, text=" + Додати дату ", bg="#00897B", fg="white",
        ...
        del_btn = tk.Label(tb, text=" - Видалити останню ", bg="#C62828", fg="white",
        ...
        tk.Label(tb, text="Загалом:", bg=C["date_nav_bg"], fg="#B0BEC5",
                 font=("Segoe UI", 10)).pack(side="right")

        # Scrollable table area
        container = tk.Frame(self.tab_finance, bg=C["bg"])
        container.pack(fill="both", expand=True)

        self._fin_canvas = tk.Canvas(container, bg=C["bg"], highlightthickness=0)
        sb_v = ttk.Scrollbar(container, orient="vertical", command=self._fin_canvas.yview)
        sb_h = ttk.Scrollbar(container, orient="horizontal", command=self._fin_canvas.xview)
        self._fin_table_frame = tk.Frame(self._fin_canvas, bg=C["bg"])

        self._fin_table_frame.bind("<Configure>",
            lambda e: self._fin_canvas.configure(scrollregion=self._fin_canvas.bbox("all")))
        self._fin_canvas_win = self._fin_canvas.create_window((0, 0), window=self._fin_table_frame, anchor="nw")
        self._fin_canvas.configure(yscrollcommand=sb_v.set, xscrollcommand=sb_h.set)

        sb_v.pack(side="right", fill="y")
        sb_h.pack(side="bottom", fill="x")
        self._fin_canvas.pack(side="left", fill="both", expand=True)

        self._fin_entries = {}  # (med_id, date_str) -> StringVar
        self._fin_col_totals = {}  # date_str -> Label
        self._fin_row_totals = {}  # med_id -> Label
    ...
    def _refresh_finance_table(self):
        ...
        tk.Label(self._fin_table_frame, text="Разом", bg="#1B5E20", fg="white",
                 font=("Segoe UI", 9, "bold"), width=10, relief="ridge"
        ...
        for ri, med in enumerate(meds):
            ...
            for ci, d in enumerate(dates):
                ...
                e = tk.Entry(self._fin_table_frame, textvariable=sv, width=10,
                             font=("Segoe UI", 9), justify="right", relief="ridge",
                             bg=cell_bg)
            ...
            rt = tk.Label(self._fin_table_frame, text="0.00", bg="#E8F5E9", fg="#1B5E20",
                          font=("Segoe UI", 9, "bold"), width=10, anchor="e", padx=6,
                          relief="ridge")
        ...
        tk.Label(self._fin_table_frame, text="Разом по даті:", bg=tot_bg, fg=C["text"],
    ...
    def _fin_recalc(self):
        ...
        for med in self._fin_meds:
            ...
            if mid in self._fin_row_totals:
                self._fin_row_totals[mid].configure(
                    text=f"{row_sum:.2f}" if row_sum else "")
            grand += row_sum
        ...
        for d in dates:
            ...
            if d in self._fin_col_totals:
                self._fin_col_totals[d].configure(
                    text=f"{col_sum:.2f}" if col_sum else "")
    ...
    def _fin_add_date(self):
        ...
        tk.Label(dlg, text="Введіть дату (ДД.ММ.РРРР):", bg=C["bg"], fg=C["text"],
    ...
    def _show_export_dialog(self):
        ...
        tk.Spinbox(range_frame, from_=1, to=31, width=3, font=("Segoe UI", 10),
                   textvariable=var_from_day).grid(row=0, column=1, padx=2)
        tk.Spinbox(range_frame, from_=1, to=12, width=3, font=("Segoe UI", 10),
                   textvariable=var_from_month).grid(row=0, column=2, padx=2)
        tk.Spinbox(range_frame, from_=2024, to=2030, width=5, font=("Segoe UI", 10),
                   textvariable=var_from_year).grid(row=0, column=3, padx=2)

        tk.Label(range_frame, text="  По:", bg=C["bg"], fg=C["text"], font=("Segoe UI", 10)).grid(
            row=0, column=4, padx=(10, 6))
        tk.Spinbox(range_frame, from_=1, to=31, width=3, font=("Segoe UI", 10),
                   textvariable=var_to_day).grid(row=0, column=5, padx=2)
        tk.Spinbox(range_frame, from_=1, to=12, width=3, font=("Segoe UI", 10),
                   textvariable=var_to_month).grid(row=0, column=6, padx=2)
        tk.Spinbox(range_frame, from_=2024, to=2030, width=5, font=("Segoe UI", 10),
                   textvariable=var_to_year).grid(row=0, column=7, padx=2)
        ...
        def set_range(days_back):
            ...
            var_from_day.set(str(s.day))
            var_from_month.set(str(s.month))
            var_from_year.set(str(s.year))
